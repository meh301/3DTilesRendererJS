...
export class EnvironmentControls extends EventDispatcher {
	...
	setTilesRenderer(tilesRenderer) {
		...
		if (this.tilesRenderer) {
			this.tilesRenderer.removeEventListener(
				"tile-visibility-change",
				this._tilesOnChangeCallback
			);
		}
	...
	attach(domElement) {
		if (this.domElement) {
			throw new Error(
				"EnvironmentControls: Controls already attached to element"
			);
		}
				...
				if (!_pointerMoveQueued) {
					...
					queueMicrotask(() => {
						...
						if (this.state === NONE || this.state === WAITING) {
							...
							if (
								Math.abs(separateDelta) > dragThreshold ||
								parallelDelta > dragThreshold
							) {
						...
						if (this.state === ZOOM) {
							const previousDist =
								pointerTracker.getPreviousTouchPointerDistance();
	...
	getPivotPoint(target) {
		...
		if (hit) {
			if (
				result === null ||
				hit.distance < result.distanceTo(raycaster.ray.origin)
			) {
	...
	update(deltaTime = Math.min(this.clock.getDelta(), 64 / 1000)) {
		...
		if (this.needsUpdate || inertiaNeedsUpdate) {
			...
			if (state === DRAG || state === ROTATE) {
				...
				this.inertiaTargetDistance = _vec
		...
		const hit = camera.isOrthographicCamera
			? null
			: (adjustHeight && this._getPointBelowCamera()) || null;
		const rotationPoint = camera.isOrthographicCamera
			? pivotPoint
			: (hit && hit.point) || null;
		this.getCameraUpDirection(_localUp);
		...
		if (
			(this.state === DRAG || this.state === ROTATE) &&
			this.actionHeightOffset !== 0
		) {
	...
	adjustCamera(camera) {
		...
		if (camera.isPerspectiveCamera) {
			...
			const hit =
				(adjustHeight &&
					this._getPointBelowCamera(camera.position, _localUp)) ||
				null;
			if (hit) {
				const dist = hit.distance;
				if (dist < cameraRadius) {
	...
	_updateInertia(deltaTime) {
		...
		if (rotationInertia.lengthSq() > 0) {
			...
			setRaycasterFromCamera(
				_ray,
				_delta.set(pixelThreshold, pixelThreshold, -1),
				camera
			);
			_ray.applyMatrix4(camera.matrixWorldInverse);
			_ray.direction.normalize();
			_ray
				.recast(-_ray.direction.dot(_ray.origin))
				.at(stableDistance / _ray.direction.z, _delta);
		...
		if (rotationInertia.lengthSq() > 0) {
			this._applyRotation(
				rotationInertia.x * deltaTime,
				rotationInertia.y * deltaTime,
				pivotPoint
			);
		}
	...
	_updateZoomDirection() {
		...
		const { domElement, raycaster, camera, zoomDirection, pointerTracker } =
			this;
		pointerTracker.getLatestPoint(_pointer);
	...
	_updateZoomPoint() {
		...
		if (
			...
			pointerTracker.getLatestPoint(_zoomPointPointer)
		) {
			mouseToCoords(
				_zoomPointPointer.x,
				_zoomPointPointer.y,
	...
	_updateRotation(deltaTime) {
		const { pivotPoint, pointerTracker, domElement, state, rotationInertia } =
			this;
		...
		if (state === ROTATE) {
			...
			_deltaPointer
				...
				.multiplyScalar((2 * Math.PI) / domElement.clientHeight);
	...
	_applyRotation(x, y, pivotPoint) {
		...
		const { camera, minAltitude, maxAltitude, rotationSpeed } = this;
