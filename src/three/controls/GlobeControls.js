...
export class GlobeControls extends EnvironmentControls {
	...
	adjustCamera(camera) {
		...
		if (camera.isPerspectiveCamera) {
			...
			const alpha = MathUtils.clamp(
				(distanceToCenter - maxRadius) / margin,
				0,
				1
			);
			...
			const elevation = Math.max(
				ellipsoid.getPositionElevation(_pos),
				MIN_ELEVATION
			);
			const horizonDistance = ellipsoid.calculateHorizonDistance(
				_latLon.lat,
				elevation
			);
	...
	_updateInertia(deltaTime) {
		...
		if (this.globeInertiaFactor !== 0) {
			...
			_ray
				.recast(-_ray.direction.dot(_ray.origin))
				.at(stableDistance / _ray.direction.z, _vec);
			_vec.applyMatrix4(camera.matrixWorld);

			setRaycasterFromCamera(
				_ray,
				_pos.set(pixelThreshold, pixelThreshold, -1),
				camera
			);
			_ray.applyMatrix4(camera.matrixWorldInverse);
			_ray.direction.normalize();
			_ray
				.recast(-_ray.direction.dot(_ray.origin))
				.at(stableDistance / _ray.direction.z, _pos);
			...
			const globeAngle =
				2 * Math.acos(globeInertia.w) * this.globeInertiaFactor;
		...
		if (this.globeInertiaFactor !== 0) {
			...
			_quaternion
				.identity()
				.slerp(globeInertia, this.globeInertiaFactor * deltaTime);
	...
	_updatePosition(deltaTime) {
		if (this.state === DRAG) {
			...
			const pivotRadius = _vec
				.copy(pivotPoint)
				.applyMatrix4(tilesGroup.matrixWorldInverse)
				.length();
			...
			if (
				pointerTracker.getMoveDistance() / deltaTime <
	...
	_updateZoom() {
		...
		const deltaAlpha = MathUtils.clamp(
			MathUtils.mapLinear(Math.abs(zoomDelta), 0, 20, 0, 1),
			0,
			1
		);
		if (this._isNearControls() || zoomDelta > 0) {
			...
			if (zoomDelta < 0 && (this.zoomPointSet || this._updateZoomPoint())) {
				...
				const upAlpha = MathUtils.clamp(
					MathUtils.mapLinear(-_zoomPointUp.dot(_toCenter), 1, 0.95, 0, 1),
					0,
					1
				);
			...
			const distanceAlpha = MathUtils.mapLinear(
				this.getDistanceToCenter(),
				transitionDistance,
				maxDistance,
				0,
				1
			...
			this._tiltTowardsCenter(
				MathUtils.lerp(0, 0.4, distanceAlpha * deltaAlpha)
			);
			this._alignCameraUpToNorth(
				MathUtils.lerp(0, 0.2, distanceAlpha * deltaAlpha)
			);
	...
	_getPerspectiveTransitionDistance() {
		...
		const fovHoriz =
			2 *
			Math.atan(Math.tan(MathUtils.DEG2RAD * camera.fov * 0.5) * camera.aspect);
		const distVert =
			ellipsoidRadius / Math.tan(MathUtils.DEG2RAD * camera.fov * 0.5);
	...
	_getVirtualOrthoCameraPosition(target, camera = this.camera) {
		...
		target
			...
			.addScaledVector(_forward, dist - orthoSize * 4);
	}
	...
	_isNearControls() {
		...
		if (camera.isPerspectiveCamera) {
			return (
				this.getDistanceToCenter() < this._getPerspectiveTransitionDistance()
			);
		} else {
